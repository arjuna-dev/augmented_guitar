...
int teensyTouchInit(uint8_t pin) {
  ...
  *portConfigRegister(pin) = PORT_PCR_MUX(0);
  ...
  TSI0_GENCS = TSI_GENCS_REFCHRG(3) | TSI_GENCS_EXTCHRG(4) | TSI_GENCS_PS(PRESCALE)
...
//int pin_array_size, int*& ptr_pin_array, int* ptr_mux_ch_original, int*& ptr_mux_ch_index){
void teensyTouchRead(int touch_array[], int touch_array_size, int*& ptr_touch_array, int pin_array[], 
...
//     print("ptr_pins_array == pins_array_end");
