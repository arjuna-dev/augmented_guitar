...
void GuitarStringMock::set_note_on_timestamp(unsigned long millis){
  _note_on_timestamp = millis;
};

void GuitarStringMock::set_current_amplitude(int amplitude){
  _current_amplitude = amplitude;
};

bool GuitarStringMock::get_note_on(){
 return _note_on;
};

int GuitarStringMock::analog_reader_right_hand(int pin){
  return _mock_sine_wave_vector[_mock_sine_wave_index++];
}

int GuitarStringMock::analog_reader_left_hand(int mux_pin){
