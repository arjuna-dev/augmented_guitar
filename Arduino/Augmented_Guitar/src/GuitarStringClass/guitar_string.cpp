...
void GuitarString::updateStringMIDIValue() {
  ...
  for (int j = 0; j < 4; j++) {
    ...
    if (is_last_fret_touched && is_fret_touched && !note_on) {
      pressed_fret = j + 1;
    }
  ...
  if (pressed_fret != last_sent_pressed_fret) {
    MIDI_lift_fret(_string_number, last_sent_pressed_fret);
    MIDI_press_fret(_string_number, pressed_fret);
    last_sent_pressed_fret = pressed_fret;
  }
