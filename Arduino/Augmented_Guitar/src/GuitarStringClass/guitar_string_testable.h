#ifndef GUITAR_STRING_TESTABLE_H
#define GUITAR_STRING_TESTABLE_H

#include "../device_specs/device_specs.h"
#include "guitar_string.h"


class GuitarStringTestable: public GuitarString {


  public:
    GuitarStringTestable(
      const int string_number = 0,
      const int input_pin = 0,
      const char open_string_note = 0,
      const int max_amplitude = 0,
      const int min_threshold = 0,
      const int max_wave_period = 0,
      int* touch_analog_values = 0,
      int* touch_reference_analog_values = 0
) : 
    GuitarString(string_number, input_pin, open_string_note, max_amplitude, min_threshold, max_wave_period, touch_analog_values, touch_reference_analog_values) {};
    bool _note_on;
    int _pressed_fret;
    int _string_number;
    int _input_pin;
    int _open_string_note;
    int _max_amplitude;
    int _min_threshold;
    int _max_wave_period;
    int _capacitance_threshold;
    int* _touch_analog_values;
    int* _touch_reference_analog_values;
    unsigned long _note_on_timestamp;
    int _current_amplitude;
    int _previous_amplitude;
    int _peak_value;
    int _last_peak_value;
    int _last_sent_pressed_fret;
    int _last_sent_note_on_fret;
};


#endif