...
#include "../teensy_touch/teensy_tsi_mock.h"
...
test(correct) {
  while (touch_reference_analog_values_mock[31] == 0) {
    tt_reference_values_mock.readNonBlocking(ptr_touch_reference_analog_values_mock, ptr_mux_pins_mock, ptr_mux_ch_mock);
  }
